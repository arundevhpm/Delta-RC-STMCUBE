...
#pragma data_alignment = 4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN uint8_t USBD_HID_KEYBOARD_ReportDesc[] __ALIGN_END =
    {
        ...
        0x81, 0x01, // 0x03   INPUT (Cnst,Var,Abs)

        0x95, 0x03, // 0x05   REPORT_COUNT (5)
        0x75, 0x01, //   REPORT_SIZE (1)
        0x15, 0x00, //   LOGICAL_MINIMUM (0)
        0x25, 0x01, // 0x65   LOGICAL_MAXIMUM (101)
        ...
        0x95, 0x01, // REPORT_COUNT (1)
        0x75, 0x05, // 0x03   REPORT_SIZE (3)
        0x91, 0x01, // 0x03   OUTPUT (Cnst,Var,Abs)

        0x95, 0x06, //   REPORT_COUNT (6)
        0x75, 0x08, //   REPORT_SIZE (8)
        0x15, 0x00, //   LOGICAL_MINIMUM (0)
        0x25, 0x73, // 0x65   LOGICAL_MAXIMUM (101)
        0x05, 0x07, //   USAGE_PAGE (Keyboard)
...
uint8_t USBD_FrameWork_AddToConfDesc(USBD_DevClassHandleTypeDef* pdev, uint8_t Speed, uint8_t* pCmpstConfDesc)
{
    ...
    case CLASS_TYPE_HID:
        ...
        case INTERFACE_HID_KEYBOARD:
            ...
            {
                /* Assign IN Endpoint */
                USBD_FrameWork_AssignEp(pdev, USBD_HID_KEYBOARD_EPIN_ADDR, USBD_EP_TYPE_INTR, USBD_HID_KEYBOARD_EPIN_HS_MPS);
            }
            else
            {
                /* Assign IN Endpoint */
                USBD_FrameWork_AssignEp(pdev, USBD_HID_KEYBOARD_EPIN_ADDR, USBD_EP_TYPE_INTR, USBD_HID_KEYBOARD_EPIN_FS_MPS);
            }
...
static void USBD_FrameWork_HID_Desc(USBD_DevClassHandleTypeDef* pdev, uint32_t pConf, uint32_t* Sze)
{
    ...
    case INTERFACE_HID_KEYBOARD:
        ...
        __USBD_FRAMEWORK_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0], 0U, (uint8_t)(pdev->tclasslist[pdev->classId].NumEps), UX_DEVICE_CLASS_HID_CLASS, 0x01U, INTERFACE_HID_KEYBOARD, 0U);
        ...
        *Sze += (uint32_t)sizeof(USBD_HIDDescTypedef);
        ...
        if (pdev->Speed == USBD_HIGH_SPEED)
        {
            ...
            __USBD_FRAMEWORK_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add, USBD_EP_TYPE_INTR, (uint16_t)pdev->tclasslist[pdev->classId].Eps[0].size, USBD_HID_KEYBOARD_EPIN_HS_BINTERVAL, USBD_HID_KEYBOARD_EPIN_FS_BINTERVAL);
        }
        else
        {
            /* Append Endpoint descriptor to Configuration descriptor */
