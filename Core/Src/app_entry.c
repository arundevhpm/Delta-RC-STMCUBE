...
//TX_SEMAPHORE SleepMode_semaphore;
static TX_THREAD    thread_ShciUserEvtProcess;

TX_SEMAPHORE semaphore_keypress_evt;
static TX_THREAD    thread_KeypressEvtProcess;
void thread_KeypressEvtProcess_entry(ULONG thread_input);

TX_SEMAPHORE semaphore_Periodic_evt;
static TX_THREAD    thread_PeriodicEvtProcess;
...
extern uint8_t KBD_mgr_timer_Id;
extern uint8_t Periodic_mgr_timer_Id;
extern void KBD_Mgr(void);
extern void Periodic_Mgr(void);
/* USER CODE END PFP */
...
void tx_application_define(void* first_unused_memory)
{
  ...
  tx_byte_pool_create(&app_byte_pool, "byte pool 0", a_memory_area, DEMO_BYTE_POOL_SIZE);


////-------------------------------USB Thread------------------------------------------
if( PowerSource == USB_POWER)
{
    memory_ptr = (VOID *)&app_byte_pool;
...
else if( PowerSource == BATTERY_POWER)
{
  ...
  tx_byte_allocate(&app_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE_LARGE+2048, TX_NO_WAIT);
  tx_thread_create(&thread_ShciUserEvtProcess,
                   ...
                   TX_AUTO_START);
//  /* Create Semaphore.  */
//  if (tx_semaphore_create(&SleepMode_semaphore, "Semaphore", 0) != TX_SUCCESS)
//  {
//    return TX_SEMAPHORE_ERROR;
//  }
}
//----------------------------Keypad thread--------------------------------
  tx_semaphore_create(&semaphore_keypress_evt,
                      "semaphore_keypress_evt",
                      0);

  tx_byte_allocate(&app_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE_LARGE, TX_NO_WAIT);
  tx_thread_create(&thread_KeypressEvtProcess,
                   "thread_KeypressEvtProcess",
                   thread_KeypressEvtProcess_entry,
                   0,
                   p_pointer,
                   ...
                   TX_NO_TIME_SLICE,
                   TX_AUTO_START);
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &KBD_mgr_timer_Id, hw_ts_Repeated, KBD_Mgr);

 //--------------------------Periodic Thread------------------------------------
  tx_semaphore_create(&semaphore_Periodic_evt,
                      "semaphore_Periodic_evt",
                      0);
  tx_byte_allocate(&app_byte_pool, (VOID**) &p_pointer, DEMO_STACK_SIZE_LARGE, TX_NO_WAIT);
  tx_thread_create(&thread_PeriodicEvtProcess,
                   "thread_PeriodicEvtProcess",
				   thread_PeriodicEvtProcess_entry,
                   0,
                   p_pointer,
                   DEMO_STACK_SIZE_LARGE,
                   16,
                   ...
                   TX_NO_TIME_SLICE,
                   TX_AUTO_START);
  HW_TS_Create(CFG_TIM_PROC_ID_ISR, &Periodic_mgr_timer_Id, hw_ts_Repeated, Periodic_Mgr);
 // --------------------------------------------------------------------------------------------
  tx_byte_allocate(&app_byte_pool, (VOID**) &p_pointer, IDLE_THREAD_STACK, TX_NO_WAIT);
  tx_thread_create(&idle_thread, "Idle Thread", idle_thread_entry, 0,
...
void idle_thread_entry(ULONG thread_input)
{
    while (1)
    {
        ...
        // Optional: Enter low-power mode if necessary
...
extern PulsePattern BuzzerBeep;
...
//		 tx_semaphore_put(&SleepMode_semaphore);
		 if(BuzzerBeep.PatternStatus == START)
		 {
			 Ueventsgrp.EventFlag_bits.BuzzBeepEvent = 1;
		 }
	 }
  switch (GPIO_Pin)
  {
