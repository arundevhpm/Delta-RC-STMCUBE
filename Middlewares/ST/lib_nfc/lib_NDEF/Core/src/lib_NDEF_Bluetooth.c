...
uint16_t NDEF_ReadBluetoothOOB( sRecordInfo_t *pRecord, Ndef_Bluetooth_OOB_t *pBluetooth )
{
  ...
  while (pData < OOBEnd)
  {
    ...
    switch (rEIR->type)
    {
      ...
      case BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_CONFIRMATION_VALUE:
     ...
     break;
      ...
      case BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_RANDOM_VALUE:
...
uint16_t NDEF_AppendBluetoothOOB( Ndef_Bluetooth_OOB_t *pBluetooth, char* RecordID )
{
  ...
  if(NDEF_BLUETOOTH_GET_OPTIONAL_MASK(pBluetooth,BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_CONFIRMATION_VALUE))
  {
    wEIR = (NDEF_EIR_t*)pData;
    wEIR->length = sizeof(pBluetooth->SimplePairingHash) + 1;
      wEIR->type = BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_CONFIRMATION_VALUE;
    NDEF_BluetoothCopy(wEIR->value, pBluetooth->SimplePairingHash,sizeof(pBluetooth->SimplePairingHash));
    pData += wEIR->length + 1;
    
  }
  
  if(NDEF_BLUETOOTH_GET_OPTIONAL_MASK(pBluetooth,BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_RANDOM_VALUE))
  {
    wEIR = (NDEF_EIR_t*)pData;
    wEIR->length = sizeof(pBluetooth->SimplePairingRandomizer) + 1;
      wEIR->type = BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_RANDOM_VALUE;
    NDEF_BluetoothCopy(wEIR->value, pBluetooth->SimplePairingRandomizer,sizeof(pBluetooth->SimplePairingRandomizer));
    pData += wEIR->length + 1;
...
uint32_t NDEF_GetBluetoothOOBLength( Ndef_Bluetooth_OOB_t *pBluetooth )
{
  ...
  length += NDEF_BLUETOOTH_GET_OPTIONAL_MASK(pBluetooth,BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_CONFIRMATION_VALUE)? sizeof(pBluetooth->SimplePairingHash) + 2 : 0 ;   
  length += NDEF_BLUETOOTH_GET_OPTIONAL_MASK(pBluetooth,BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_RANDOM_VALUE)? sizeof(pBluetooth->SimplePairingRandomizer) + 2 : 0 ;  
