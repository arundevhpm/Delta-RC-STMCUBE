...
typedef enum {
  ...
  BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_CONFIRMATION_VALUE = 0x22,
  BLUETOOTH_EIR_LE_SECURE_CONNECTIONS_RANDOM_VALUE = 0x23,
...
#define NDEF_BLUETOOTH_SET_OPTIONAL_MASK(bt,option)   do { if (option < 32) { bt->OptionalMask |= (1<<(option)); } else \
                                                            { bt->LeOptionalMask |= (1<<(option & 0x1F));} } while (0)  

/** @brief Helper macro to unset a particular EIR in the EIR mask of a `Ndef_Bluetooth_OOB_t` instance.
  * @param bt     Pointer to a `Ndef_Bluetooth_OOB_t` struct
  * @param option Index of the option based on the `Ndef_Bluetooth_Eir_Types_t` */
#define NDEF_BLUETOOTH_UNSET_OPTIONAL_MASK(bt,option) do { if (option < 32) { bt->OptionalMask &= ~(1<<(option)); } else \
                                                            { bt->LeOptionalMask &= ~(1<<(option & 0x1F));}} while (0)  

/** @brief Helper macro to know if a particular EIR is present in the EIR mask of a `Ndef_Bluetooth_OOB_t` instance.
  * @param bt     Pointer to a `Ndef_Bluetooth_OOB_t` struct
  * @param option Index of the option based on the `Ndef_Bluetooth_Eir_Types_t` */
#define NDEF_BLUETOOTH_GET_OPTIONAL_MASK(bt,option) ((option < 32)?((bt->OptionalMask & (1<<(option))) >>  (option)) : \
                                                         ((bt->LeOptionalMask & (1<<(option & 0x1F))) >>  (option & 0x1F)))

/** @brief Helper macro to create a mask for a particular EIR.
  * @param option Index of the option based on the `Ndef_Bluetooth_Eir_Types_t` */
#define NDEF_BLUETOOTH_OPTION(option)               ((option < 32)? (1<<(option)) : \
                                                          (1<<(option & 0x1F)))
...
typedef struct
{
  ...
  uint32_t                LeOptionalMask;                     /**< This Mask is used to keep track of LE EIRs (index > 32) present in the OOB: each bit index corresponds to a `Ndef_Bluetooth_Eir_Types_t` value. */
  ...
  /* for br/edr only */
    uint8_t                 SimplePairingHash[16];            /**< *For Br/Edr *: Simple Pairing Hash from BLUETOOTH_EIR_SIMPLE_PAIRING_HASH.
                                                                   *For LE     *: Le connection confirmation value.*/
    uint8_t                 SimplePairingRandomizer[16];      /**< *For Br/Edr *: Simple Pairing Randomizer from BLUETOOTH_EIR_SIMPLE_PAIRING_RANDOMIZER.
                                                                   *For LE     *: Le connection random value.*/
